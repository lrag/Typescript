...
import { PeliculaModel } from '../modelo/activeRecords/PeliculaModel' // Importamos el modelo de Mongoose
...
export class EndpointPeliculas {
    ...
    private async listar(request: FastifyRequest, reply: FastifyReply) {
        // Uso directo de Mongoose (Active Record)
    ...
    */
    ...
    private async buscarPorId(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
        try {
            ...
            const peliculaEncontrada = await PeliculaModel.findById(request.params.id)
