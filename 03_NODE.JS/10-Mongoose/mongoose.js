...
function pruebasMongoose(){
    ...
    usuario.save()
        ...
        .then( usuarioInsertado => {
            ...
            return Usuario.findById("65f34ef0db48794ded6d5ab6")
        })
        .then( usuarioEncontrado => {
            ...
            */
            ...
            return Usuario.findByIdAndDelete("65f34ef0db48794ded6d5ab4")
        } )
        .then( resultado => {
            console.log("Usuario borrado")
...
function factoriaDePrototipos(esquema){
    let Prototipo = function(){
        for(let a=0; a<esquema.length; a++){
            let propiedad = esquema[a]
            this[propiedad] = ""
        }
    }
    //funciones 'dinámicas'
    Prototipo.prototype.save = function(){
        console.log("Pa la base de datos que voy!")
    }
    //funciones 'estáticas'
    Prototipo.findById = function(id){
        console.log('Buscando el documento con id '+id)
    }

    return Prototipo
}

let Coche = factoriaDePrototipos(['marca','modelo', 'fabricante'])
let coche = new Coche()
console.log(coche)
coche.save()
